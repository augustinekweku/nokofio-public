  ...
  async function getProfile() {
    if (userProfile?.username) {
      ...
      if (res.data.results.data) {
        updateState({
          isLoading: false,
          paymentLinks: res.data.results.data.supportMeAmounts,
        });
      }
    }
  }
      ...
      <MainRow>
        <MainLeft>
          ...
          <InnerLeft>
            ...
            >
              <Box flexGrow={1}>
                <BackNavTitle title={"Payment Links"} />
              </Box>
              <Button
                onClick={() => {
                  updateState({ openPaymentLinkDetails: true });
                }}
                variant={"outlined"}
                endIcon={<Add />}
              >
